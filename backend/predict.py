...
def predict_with_saved_model(data, model_path='./output/hepatitis_model.pkl', feature_columns_path='./output/feature_columns.json'):
    """
    ...
    if 'Symptoms' in data.columns:
        ...
        common_symptoms = ['jaundice', 'fatigue', 'nausea', 'vomiting', 'fever', 'pain']
        for symptom in common_symptoms:
            data[f'has_{symptom}'] = data['Symptoms'].str.lower().str.contains(symptom, regex=False).astype(int)
    
    # Process DiagnosisDate
    if 'DiagnosisDate' in data.columns:
        data['DiagnosisDate'] = pd.to_datetime(data['DiagnosisDate'], errors='coerce')
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(json.dumps({
            ...
            "predictions": results.head(10).to_dict(orient='records'),
